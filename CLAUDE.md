...
**不需要手工查 series 和尺寸** —— 脚本用 `showinf` 自动识别主切片（展平后像素数最大的 series）并读出 `FULL_W/FULL_H`。输出落在 `/mnt/data4/sym/svs_work/<切片名>_final.svs`。

**唯一需要人工确认的**：`--dry-run` 打印出的 series 编号与尺寸是否合理（主切片应是整张图里最大的那个）。不对就用 `--series N` / `--full-width` / `--full-height` 覆盖。

中途看进度：`tail -f /mnt/data4/sym/svs_work/convert.log`（Phase 1 无进度行，看输出文件大小增长）。
...
3. **路径。** 项目实际位于 `/mnt/data4/sym/svs_format`，另有同目录软链 `/home/sym/workspace/svs_format`（`readlink -f` 解析到前者）。旧文档写的 `/home/sym/svs_format/` **不存在**。
...
```
/mnt/data4/sym/svs_format/                 # 项目本体（软链 /home/sym/workspace/svs_format 指向此处）
├── CLAUDE.md                      # 本文件
...
/mnt/data4/sym/svs_work/           # 输出（脚本默认 WORK_DIR）
...
`/mnt/data4/sym/svs_work/FKBP4_final.svs`，2.98 GB，全程 27 分钟：
...
- `WORK_DIR=/mnt/data4/sym/svs_work`；中间产物与输出按输入名派生 → `<切片名>_base.tif` / `_pyramid.svs` / `_final.svs`，多张切片互不覆盖。
...
/opt/mambaforge/envs/rnaseq/bin/python3 reencode_rgb_jpeg.py \
    --input  /mnt/data4/sym/svs_work/FKBP4_pyramid.svs \
    --output /mnt/data4/sym/svs_work/FKBP4_final.svs \
...
```bash
SVS=/mnt/data4/sym/svs_work/FKBP4_final.svs
